...
export const fleet = async (theUser: User, lang: string) => {
  try {
    let userInput: string;
    while (userInput !== "z" && userInput !== "b") {
      console.clear();
      log(titleLog(`${texts.default[lang].fleet.title}`));
      texts.default[lang].fleet.operations.forEach(
      ...
      const findOperation = texts.default[lang].fleet.operations.find(
        (item: {
          input: string;
          label: string;
          logged: number;
