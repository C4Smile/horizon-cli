...
export const dock = async (theUser: User, lang: string) => {
  try {
    let userInput: string;
    while (userInput !== "z" && userInput !== "b") {
      console.clear();
      log(titleLog(`${texts.default[lang].dock.title}`));
      texts.default[lang].dock.operations.forEach(
      ...
      const findOperation = texts.default[lang].dock.operations.find(
        (item: {
          input: string;
          label: string;
          logged: number;
