...
export const routes = async (theUser: User, lang: string) => {
  try {
    let userInput: string;
    while (userInput !== "z" && userInput !== "b") {
      console.clear();
      log(titleLog(`${texts.default[lang].routes.title}`));
      texts.default[lang].routes.operations.forEach(
      ...
      const findOperation = texts.default[lang].routes.operations.find(
        (item: {
          input: string;
          label: string;
          logged: number;
