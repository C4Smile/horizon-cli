...
export const map = async (theUser: User, lang: string) => {
  try {
    let userInput: string;
    while (userInput !== "z" && userInput !== "b") {
      console.clear();
      log(titleLog(`${texts.default[lang].map.title}`));
      texts.default[lang].map.operations.forEach(
      ...
      const findOperation = texts.default[lang].map.operations.find(
        (item: {
          input: string;
          label: string;
          logged: number;
